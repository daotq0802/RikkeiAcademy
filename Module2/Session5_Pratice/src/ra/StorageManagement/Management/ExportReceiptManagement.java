...
public class ExportReceiptManagement {
   public static int billManagementMenu(Scanner sc) {
   ...
   public static int getReceipt(String receiptID) {
      int index = -1;
      for (int i = 0; i < StoreSystem.ordersExport.size(); i++) {
         if (StoreSystem.ordersExport.get(i).equals(receiptID)) {
            index = i;
         }
      }
      return index;
   ...
   public static void addReceipt(Scanner sc) {
      Order[] addArray = new Order[StoreSystem.ordersExport.size() + 1];
      for (int i = 0; i < addArray.length; i++) {
         if (i < StoreSystem.ordersExport.size()) {
            addArray[i] = StoreSystem.ordersExport.get(i);
         } else {
      ...
      StoreSystem.ordersExport.add(addArray[addArray.length - 1]);
   }

   public static void displayReceipt() {
      if (StoreSystem.ordersExport.size() > 0) {
   ...
   public static void updateReceipt(Scanner sc) {
      ...
      System.out.println(StoreSystem.LINE);
      for (int i = 0; i < StoreSystem.ordersExport.size(); i++) {
         System.out.printf("%d. Hoá đơn %s\n", i + 1, StoreSystem.ordersExport.get(i).getOrderID());
      }
      while (true) {
         try {
            ...
            if (choice <= 0) {
               ...
               while (true) {
                  try {
                     ...
                     switch (selection) {
                        case 1:
                           System.out.println("Số lượng hiện tại: " + StoreSystem.ordersExport.get(choice - 1).getQuantity());
                           ...
                           try {
                              ...
                              if (quantity <= 0) {
                                 ...
                                 StoreSystem.ordersExport.get(choice - 1).setQuantity(quantity);
                                 System.out.println("Cập nhật thành công!");
                                 StoreSystem.ordersExport.get(choice - 1).setUpdatedUserID(StoreSystem.loginID);
                                 StoreSystem.ordersExport.get(choice - 1).setUpdated(Order.inputTimeOfAction());
                                 return;
                              }
                           } catch (NumberFormatException e) {
                        ...
                        case 2:
                           System.out.println("Đơn giá hiện tại: " + StoreSystem.ordersExport.get(choice - 1).getPrice());
                           ...
                           try {
                              ...
                              if (price <= 0) {
                                 ...
                                 StoreSystem.ordersExport.get(choice - 1).setPrice(price);
                                 System.out.println("Cập nhật thành công!");
                                 StoreSystem.ordersExport.get(choice - 1).setUpdatedUserID(StoreSystem.loginID);
                                 StoreSystem.ordersExport.get(choice - 1).setUpdated(Order.inputTimeOfAction());
                                 return;
                        ...
                        case 3:
                           if (StoreSystem.ordersExport.get(choice - 1).getStatus().equals("Hoạt động")) {
                              ...
                              while (true) {
                                 try {
                                    ...
                                    if (yes <= 0 || yes > 2) {
                                       ...
                                       switch (yes) {
                                          case 1:
                                             StoreSystem.ordersExport.get(choice - 1).setStatus("Bị huỷ");
                                             System.out.println("Cập nhật thành công!");
                                             StoreSystem.ordersExport.get(choice - 1).setUpdatedUserID(StoreSystem.loginID);
                                             StoreSystem.ordersExport.get(choice - 1).setUpdated(Order.inputTimeOfAction());
                                             return;
                              ...
                              while (true) {
                                 try {
                                    ...
                                    if (yes <= 0 || yes > 2) {
                                       ...
                                       switch (yes) {
                                          case 1:
                                             StoreSystem.ordersExport.get(choice - 1).setStatus("Hoạt động");
                                             System.out.println("Cập nhật thành công!");
                                             StoreSystem.ordersExport.get(choice - 1).setUpdatedUserID(StoreSystem.loginID);
                                             StoreSystem.ordersExport.get(choice - 1).setUpdated(Order.inputTimeOfAction());
                                             return;
   ...
   public static void receiptExportMenuSelect(Scanner sc) {
      while (true) {
         int input = billManagementMenu(sc);
         switch (input) {
            case 1:
               addReceipt(sc);
               break;
