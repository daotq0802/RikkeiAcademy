function addNewCourse() {
  let dataIdIndex = dataCourse.findIndex(
    (course) => course.id == courseId.value
  );
  let dataNameIndex = dataCourse.findIndex(
    (course) => course.name == courseName.value
  );
  let courseStatus = () => {
...
function updateSelectedCourse() {
  ...
  if (dataCourse[dataIdIndex].name == courseName.value && dataNameIndex != -1) {
    ...
    dataCourse[dataIdIndex].name != dataCourse.value &&
    dataNameIndex == -1
  ) {
    dataCourse[dataIdIndex].name = courseName.value;
    dataCourse[dataIdIndex].time = courseTime.value;
...
function openDeleteCourse(index) {
  ...
  document.querySelector(
    ".delete-course-modal p"
  ).textContent = `Bạn chắc chắn muốn xoá ${dataCourse[index].name}`;
...
document.getElementById("delete-Course").addEventListener("click", () => {
  const validClass = dataClass.findIndex(
    (classRoom) => classRoom.courseName == dataCourse[deleteID.value].name
  );
  if (validClass != -1) {
    showToast("Khoá học hiện đang có lớp");
  } else {
